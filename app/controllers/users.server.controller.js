...
// For instance, to retrieve only the usernames and e-mails of your users, 
// you would modify your call to look like the following lines of code:

// User.find({}, 'username email', function(err, users) {
//      ...
// });

// For instance, to paginate through the users collection and retrieve only a 
// subset of your users collection, you can use the skip and limit options as follows:
     ...
     if (!req.user) {
       ...
       user.save(function(err) {
         if (err) {
           var message = getErrorMessage(err);
           req.flash('error', message);
           return res.redirect('/CntrlPanel/signup');
         }
         req.login(user, function(err) {
           if (err) return next(err);
           return res.redirect('/CntrlPanel');
